...
ansible-inventory -i /home/$USER/myazure_rm.yml --list > azure_resource_list.json

# Append all IP of the host with tag_environment_app
LENGTH=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq '.tag_environment_app.hosts | length')
for (( i=0 ; i < $LENGTH ; i++ ));
do
  temp=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq .tag_environment_app.hosts[$i])
  temp="${temp%\"}"
  temp="${temp#\"}"
  temp=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq ._meta.hostvars.$temp.ansible_host)
  temp="${temp%\"}"
  temp="${temp#\"}"
...
LENGTH=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq '.tag_environment_web.hosts | length')
for (( i=0 ; i < $LENGTH ; i++ ));
do
  temp=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq .tag_environment_web.hosts[$i])
  temp="${temp%\"}"
  temp="${temp#\"}"
  temp=$(ansible-inventory -i /home/$USER/myazure_rm.yml --list | jq ._meta.hostvars.$temp.ansible_host)
  temp="${temp%\"}"
